...
i = 0
#业务流
class changedealpsw(unittest.TestCase):
    def setUp(self):
        self.driver = basedriver()

    def test_changedealpsw(self):
        try:
            global i
            try:
            ...
            publicmethods(self.driver).wait(page.ChangeDealPsw.修改资金账户交易密码['type'],5,
                                            page.ChangeDealPsw.修改资金账户交易密码['value'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.修改资金账户交易密码text['type'],
                                                   page.ChangeDealPsw.修改资金账户交易密码text['value']).click()
            publicmethods(self.driver).wait(page.ChangeDealPsw.原交易密码['type'], 5,
                                            page.ChangeDealPsw.原交易密码['value'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.原交易密码['type'],
                                                   page.ChangeDealPsw.原交易密码['value'])\
                .send_keys(dataes.FundAccountPsw.资金账号通用密码['psw'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.新交易密码['type'],
                                                   page.ChangeDealPsw.新交易密码['value'])\
                .send_keys(dataes.FundAccountPsw.资金账号通用密码['psw'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.确认新密码['type'],
                                                   page.ChangeDealPsw.确认新密码['value']) \
                .send_keys(dataes.FundAccountPsw.资金账号通用密码['psw'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.确定['type'],
                                                   page.ChangeDealPsw.确定['value']).click()
            publicmethods(self.driver).wait(page.ChangeDealPsw.密码修改成功['type'],5,
                                            page.ChangeDealPsw.密码修改成功['value'])
            publicmethods(self.driver).findelement(page.ChangeDealPsw.完成['type'],
                                                   page.ChangeDealPsw.完成['value']).click()
            try:
                # 查询能否找到修改资金账户交易密码，能找到则修改成功，否则失败
                publicmethods(self.driver).wait(page.ChangeDealPsw.修改资金账户交易密码['type'], 5,
                                                page.ChangeDealPsw.修改资金账户交易密码['value'])
                publicmethods(self.driver).getScreenShot('修改资金账户交易密码', '成功')
                time.sleep(1)
                # 返回至我的页面
            ...
            publicmethods(self.driver).getScreenShot('修改资金账户交易密码', '失败')
            # 报错信息
            raise
