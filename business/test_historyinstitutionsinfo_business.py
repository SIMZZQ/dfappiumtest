...
#业务流
class historyinstitutionsinfo(unittest.TestCase):
    def setUp(self):
        self.driver = basedriver()

    def test_historyinstitutionsinfo(self):
        try:
            try:
            ...
            publicmethods(self.driver).wait(page.MinePage.机构户我的信息['type'], 5, page.MinePage.机构户我的信息['value'])
            publicmethods(self.driver).findelement(page.MinePage.机构户我的信息['type'], page.MinePage.机构户我的信息['value'])\
                .click()
            try:
                ...
                time.sleep(1)
                publicmethods(self.driver).getScreenShot('机构账号历史登录信息','成功')
            ...
            publicmethods(self.driver).getScreenShot('机构账号历史登录信息','失败')
            # 报错信息
            raise
