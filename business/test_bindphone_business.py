...
exist = 0
#业务流
class bindphone(unittest.TestCase):
    def setUp(self):
    ...
    def test_bindphone(self):
        try:
            ...
            publicmethods(self.driver).findelement(page.BindPhone.绑定手机号['type'],
                                                   page.BindPhone.绑定手机号['value']).click()
            publicmethods(self.driver).wait(page.BindPhone.手机号输入框['type'],5,page.BindPhone.手机号输入框['value'])
            ...
            while exist == 0:
                try:
                    ...
                    publicmethods(self.driver).findelement(page.BindPhone.发送验证码['type'],
                                                           page.BindPhone.发送验证码['value']).click()
                    publicmethods(self.driver).wait(page.ToastPage.手机号已绑定['type'],2,
            ...
            publicmethods(self.driver).findelement(page.BindPhone.确定['type'],page.BindPhone.确定['value']).click()
            try:
                # 判断能否找到手机号绑定成功，找到了则绑定成功，否则失败
                publicmethods(self.driver).wait(page.ToastPage.手机号绑定成功['type'],10,
